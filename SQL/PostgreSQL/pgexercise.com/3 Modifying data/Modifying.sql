...
/* We wouldn't have to use a subquery if  
facid was defined as the PK or
AUTO INCREAMENT could have been used (SERIAL type) with or without PK
*/
...
initial outlay was 10000 rather than 8000: you need to alter the data to 
fix the error.
*/
...
more than the first one. Try to do this without using constant values for 
the prices, so that we can reuse the statement if we want to.
*/

UPDATE cd.facilities SET (membercost, guestcost) = (
  SELECT membercost * 1.1, guestcost * 1.1 
  FROM cd.facilities WHERE name = 'Tennis Court 1'
  )
...
/*
We want to remove member 37, who has never made a booking, from our 
database. How can we achieve that?
*/
